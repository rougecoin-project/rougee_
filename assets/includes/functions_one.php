...
function Wo_RegisterPost($re_data = array('recipient_id' => 0)) {
    ...
    if (!empty($re_data['postText'])) {
        ...
        if (preg_match('%(?:youtube(?:-nocookie)?\.com/(?:shorts/|[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $re_data['postText'], $youtubeMatch)) {
            $re_data['postYoutube'] = Wo_Secure($youtubeMatch[1]);
            $re_data['postText'] = preg_replace('%' . preg_quote($youtubeMatch[0], '%') . '%i', "", $re_data['postText']); // Using preg_quote to safely use the matched URL in a regex
                
            $link_regex = '/(http\:\/\/|https\:\/\/|www\.)([^\ ]+)/i';
            preg_match_all($link_regex, $re_data['postText'], $linkMatches);
            foreach ($linkMatches[0] as $linkMatch) {
                $match_url = strip_tags($linkMatch);
                $syntax = ''; // you might want to replace with some other syntax or just keep it empty as it is now
                $re_data['postText'] = str_replace($linkMatch, $syntax, $re_data['postText']);
            }
            $is_there_video = true;
        }
        
        if (Wo_IsUrl($wo['config']['playtube_url']) && preg_match('#' . $playtube_root . '\/(?:shorts|embed)\/(.*)#i', $re_data['postText'], $match)) {
        ...
        if (preg_match('%(?:https?://)on\.soundcloud\.com/([\w\-]+)%im', $re_data['postText'], $match) || preg_match('%(?:https?://)(?:www\.)?soundcloud\.com/([\-a-z0-9_]+/[\-a-z0-9_]+)%im', $re_data['postText'], $match)) {
            $track_url = $match[0];
            
            // Define the oEmbed URL
            $oembed_url = 'https://on.soundcloud.com/oembed';
            ...
            $response = @file_get_contents($oembed_url . '?' . $params);
        
            // Check if the request was successful and extract the embed code
            if ($response !== false) {
                $json_response = json_decode($response, true);
                if (isset($json_response['html'])) {
                    $re_data['postSoundCloud'] = $json_response['html'];
                    $is_there_video = true;
                }
            } else {
                // Handle error (you can expand on this if needed)
                error_log("Error fetching SoundCloud oEmbed for URL: " . $track_url);
            }
        }
...
function Wo_GetPosts($data = array('filter_by' => 'all', 'after_post_id' => 0, 'page_id' => 0, 'group_id' => 0, 'publisher_id' => 0, 'limit' => 10, 'event_id' => 0, 'ad-id' => 0)) {
    global $wo, $sqlConnect;
    ...
    if (isset($Wo_publisher['user_id'])) {
        ...
        switch ($data['filter_by']) {
            ...
            case 'maps':
                ...
                break;
            case 'ai_post':
                $query_text .= " AND `ai_post` = 1"; // Assuming `ai_post` is a column in your posts table that can be 1 or 0    
                break;        
...
function getMidJeournyImage($text, $size, $num_outputs = 1) {
    ...
    $model = $wo['config']['midjeourny_model'];
    $sizes = getMidJeournyModels($model);
    if (!in_array($size, $sizes)) {
        // Ensure the provided size is valid for the model.
        throw new Exception("Invalid size for the selected model.");
    }

    $url = 'https://d5da2aae65d2c496b2.gradio.live/sdapi/v1/txt2img';
    ...
    if (isset($result->images)) {
        ...
        throw new Exception($result->detail[0]->msg);
    } else {
