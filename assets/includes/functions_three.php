...
function Wo_RegisterPoint($post_id, $type, $action = '+', $user_id = 0, $post_user_id = 0) {
    ...
    if (!empty($user_id) && is_numeric($user_id) && $user_id > 0) {
        ...
        $post_user_id = Wo_GetUserIdFromPostId($post_id);
    } else {
...
function Wo_GetFriendsStatus($data_array = array('limit' => 8, 'user_id' => 0, 'offset' => 0)) {
    ...
    $query = "SELECT DISTINCT user_id,title,description,posted,expire,thumbnail,
              (SELECT MAX(us.id) FROM " . T_USER_STORY . " us WHERE us.user_id = " . T_USER_STORY . ".user_id) AS id  
              FROM " . T_USER_STORY . " 
              WHERE user_id IN (SELECT user_id FROM " . T_USERS . " WHERE active = '1') 
              $offset_query 
              ORDER BY id DESC LIMIT " . $data_array['limit'];

    $query_run = mysqli_query($sqlConnect, $query);

    // Initialize an array to store grouped stories by user
    $grouped_stories = array();

    while ($fetched_data = mysqli_fetch_assoc($query_run)) {
        $user_id = $fetched_data['user_id'];

        // Check if the user's stories are already in the grouped array
        if (!isset($grouped_stories[$user_id])) {
            $grouped_stories[$user_id] = $fetched_data;
            $grouped_stories[$user_id]['stories'] = array();
        }

        // Append the current story to the user's stories
        $grouped_stories[$user_id]['stories'][] = $fetched_data;
    }

    // Convert the grouped stories to a flat array
    foreach ($grouped_stories as $user_id => $user_stories) {
        // Check if the logged-in user has not seen any of the user's stories
        $not_seen = 0;
        foreach ($user_stories['stories'] as $story) {
            if (!$db->where("(user_id = $user_id AND id NOT IN (SELECT story_id FROM " . T_STORY_SEEN . " WHERE user_id = " . $wo['user']['id'] . "))")->has(T_USER_STORY)) {
                $not_seen = 1;
                break; // At least one story is not seen, break the loop
            }
        }

        $user_stories['have_not_seen'] = $not_seen;
        $user_stories['user_data'] = Wo_UserData($user_id);

        // Use the last story's thumbnail
        $last_story = end($user_stories['stories']);
        if (empty($last_story['thumbnail'])) {
            $user_stories['thumb']['filename'] = $user_stories['user_data']['avatar_org'];
        } else {
            $user_stories['thumb'] = array();
            $user_stories['thumb']['filename'] = $last_story['thumbnail'];
        }

        $user_stories['thumb']['filename'] = Wo_GetMedia($user_stories['thumb']['filename']);
        $user_stories['videos'] = Wo_GetStoryMedia($last_story['id'], 'video');
        $user_stories['is_owner'] = ($user_id == $wo['user']['id'] || Wo_IsAdmin() || Wo_IsModerator()) ? true : false;

        // Append the grouped user stories to the result
        $data[] = $user_stories;
    }
...
function Wo_GetFriendsStatusAPI($data_array = array('limit' => 8, 'user_id' => 0, 'offset' => 0)) {
    ...
    $query = "SELECT DISTINCT user_id,title,description,posted,expire,thumbnail,
              ...
              WHERE user_id IN (SELECT user_id FROM " . T_USERS . " WHERE active = '1') 
              $offset_query 
              ORDER BY user_id DESC LIMIT " . $data_array['limit'];
