<?php
if ($wo['config']['website_mode'] != 'facebook') {
    header("Location: " . Wo_SeoLink('index.php?link1=welcome'));
    exit();
}

$two_weeks_ago = strtotime("-4 weeks");

$explore_posts = $db
    ->where('postPrivacy','0')
    ->where('active',1)
    ->where('time', $two_weeks_ago, '>')
    ->where("((postFile LIKE '%.jpg%' || postFile LIKE '%.jpeg%' || postFile LIKE '%.png%' || postFile LIKE '%.gif%' || postFile LIKE '%.mp4%' || postFile LIKE '%.mkv%' || postFile LIKE '%.avi%' || postFile LIKE '%.webm%' || postFile LIKE '%.mov%' || postFile LIKE '%.m3u8%' || postSticker != '' || postPhoto != '' || album_name != '' || multi_image = '1'))")
    ->orderBy('RAND()')
    ->get(T_POSTS, 36, array('id'));
